...
from model.load_model import get_uncompiled_model, get_compiled_model, get_trained_model_h5, get_trained_model_ckpt
...
def tuning(args):
    ...
    summary_filepath=f"./logs/tuning_0.1"
    callbacks = [
        tf.keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(summary_filepath,"checkpoint/run_{epoch}"),
        ...
        MultiValidationCallback([datasets[1], datasets[3], datasets[4]])
    ]

    hist=model.fit(datasets[5], validation_data=datasets[6], class_weight=dict(enumerate(class_weights[3])), 
            epochs=200, callbacks=callbacks, verbose=0)
    
    model.save(os.path.join(summary_filepath,'model.h5'))
